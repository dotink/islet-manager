...
<div class="islet manager">
	<div class="toolbar">
    <button v-if="!focusedContainer || sorting || view == 'add' || view == 'remove'" class="disabled parent">Parent</button>
		<button v-if="focusedContainer && !sorting && view != 'add' && view != 'remove'" class="parent" v-on:click="focus(focusedContainer.parentNode)">Parent</button>

    <button v-if="!focusedContainer || selected || view == 'add' || view == 'remove'" class="disabled add">Add</button>
		<button v-if="focusedContainer && !selected && view != 'add' && view != 'remove'" class="add" v-on:click="show('add')">Add</button>
    ...
    <div v-bind:class="{ active: view == 'help' }" class="help">
      <div v-if="!focusedContainer" class="message">
        <p>
      ...
      <div v-if="focusedContainer" class="message">
		...
		<div v-bind:class="{ active: view == 'list' }" class="list">
			<ol>
				<li v-bind:class="{ selected: component.selected }" v-for="component in components" v-on:dragend="sort(component)" v-on:dragstart="sort(component)" v-on:click="select(component, $event)" v-on:dblclick="focus(component.$el)">
					<h1 v-if="canFocus(component.$el)" title="Click to select or deselect.  Double click to manage its interior elements.">{{ component.label }} <span >↴</span></h1>
          <h1 v-if="!canFocus(component.$el)" title="Click to select or deselect this element for removal.">{{ component.label }}</h1>
					<div class="preview">
